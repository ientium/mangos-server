...
namespace LuaPlayer
{
	...
	int IsFlying(lua_State* L, Player* player) // enable for unit when mangos support it
	{
		Eluna::Push(L, player->IsTaxiFlying());
		return 1;
	}
    /**
	...
	/**
	* Returns the [Player]s VIP高级积分
	*
	* @return uint32 coinage
	*/
	int getVipCoin(lua_State* L, Player* player)
	{
		Eluna::Push(L, player->getVipInfo(4));
		return 1;
	}
	/**
	* Returns the [Player]s VIP最高等级
	*
	* @return uint32 coinage
	*/
	int getMaxLevel(lua_State* L, Player* player)
	{
		Eluna::Push(L, STRONG_MAX_LEVEL);
		return 1;
	}
	/**
	* Returns the [Player]s 瞬飞剩余时间
	*
	* @return uint32 coinage
	*/
	int getVipFlyingTime(lua_State* L, Player* player)
	{
		Eluna::Push(L, player->getVipInfo(2));
		return 1;
	}
	/**
	* Returns the [Player]s 未转化时间
	*
	* @return uint32 coinage
	*/
	int getVipTimeToCoin(lua_State* L, Player* player)
	{
		Eluna::Push(L, player->getVipInfoTimeToCoin());
		return 1;
	}
	/**
	* Returns the [Player]s 转化时间为积分
	*
	* @return uint32 coinage
	*/
	int setVipCoin(lua_State* L, Player* player)
	{
		uint32 entry = Eluna::CHECKVAL<uint32>(L, 2);
		Eluna::Push(L, player->setVipMemberCoin(entry));
		return 1;
	}
	/**
	* Returns the [Player]s 转化时间为积分
	...
	* @return uint32 coinage
	*/
	int setUpdateFlyingTime(lua_State* L, Player* player)
	{
		uint32 tentry = Eluna::CHECKVAL<uint32>(L, 2);
		uint32 entry = Eluna::CHECKVAL<uint32>(L, 3);
		Eluna::Push(L, player->setUpdateVIPFlyingTime(tentry, entry));
		return 1;
	}
	/**
	* Returns the [Player]s 用户当前等级
	*
	* @return uint32 level
	*/
	int GetLevel(lua_State* L, Player* player)
	{

		Eluna::Push(L, player->GetInfoLevel());
		return 1;
	}
	/**
	* Returns the [Player]s 用户当前等级
	...
	* @return uint32 level
	*/
	int SetLevelUp(lua_State* L, Player* player)
	{

		uint16 addlevel = Eluna::CHECKVAL<uint32>(L, 2);
		uint32 costcoin = Eluna::CHECKVAL<uint32>(L, 3);
		uint16 ttype = Eluna::CHECKVAL<uint32>(L, 4);
		if (ttype == 0) {
			Eluna::Push(L, player->LevelUp(addlevel, costcoin));
		}
		else {
			Eluna::Push(L, player->LevelUp(addlevel));
		}
		return 1;
	}
