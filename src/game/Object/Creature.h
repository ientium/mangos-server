...
struct VendorItem
{
	...
	//VIPInfo商店设置初始化
	VendorItem(uint32 _item, uint32 _maxcount, uint32 _incrtime, uint16 _conditionId, uint32 excost, uint16 itemtype, uint16 _itclass)
		: item(_item), maxcount(_maxcount), incrtime(_incrtime), conditionId(_conditionId), excost(excost), itemtype(itemtype), itclass(_itclass) {}
	VendorItem(uint32 _item, uint32 _maxcount, uint32 _incrtime, uint16 _conditionId)
		: item(_item), maxcount(_maxcount), incrtime(_incrtime), conditionId(_conditionId), excost(0), itemtype(0), itclass(0) {}
	uint32 item;
...
struct VendorItemData
{
	...
	//VIPInfo商店设定
	void AddItem(uint32 item, uint32 maxcount, uint32 ptime, uint16 conditonId, uint32 excost, uint16 itemtype, uint16 _itclass)
	{
		m_items.push_back(new VendorItem(item, maxcount, ptime, conditonId, excost, itemtype, _itclass));
	}
