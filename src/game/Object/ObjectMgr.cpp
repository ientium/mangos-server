...
void ObjectMgr::LoadVendors(char const* tableName, bool isTemplates)
{
	...
	if (isTemplates) {
		...
		result = WorldDatabase.PQuery("SELECT entry, item, maxcount, incrtime, condition_id, excost,itemtype,class FROM %s", tableName);

	}
	else {
		result = WorldDatabase.PQuery("SELECT entry, item, maxcount, incrtime, condition_id FROM %s", tableName);
	}
    ...
    do
    {
		...
		uint16 itclass = 0;
		if (isTemplates) {
			excost = fields[5].GetUInt32();                                          //额外花费的点数
			itemtype = fields[6].GetUInt16();                                           //商店类型
			itclass = fields[7].GetUInt16();                                           //商店类型
		}
		...
		if (isTemplates) {
			vList.AddItem(item_id, maxcount, incrtime, conditionId, excost, itemtype, itclass);
		}
		else {
